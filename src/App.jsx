...
export default function App() {
  ...
  return <div className="min-h-screen bg-slate-100 text-slate-900"><div className="mx-auto flex max-w-[1500px] gap-6 p-6">
    <aside className="w-72 rounded-2xl bg-brand-charcoal p-6 text-white shadow-2xl"><LogoBlock />
      ...
      <nav className="mt-6 space-y-2">{navItems.map((item) => <button key={item} onClick={() => setPage(item)} className={`w-full rounded-lg px-3 py-2 text-left ${page === item ? 'bg-brand-orange font-semibold text-white' : 'bg-slate-800 hover:bg-slate-700'}`}>{item}</button>)}</nav>
    </aside>
    ...
    <main className="flex-1 space-y-6">
      ...
      {page === 'Create Invoice' && <section className="space-y-4"><h2 className="text-3xl font-bold">Create Invoice</h2>
        ...
        <InvoicePreview {...{ customerName, invoiceNumber, invoiceDate, invoiceItems, subtotal, tax, freight, total, selectedPayment, setSelectedPayment, terms, taxRate, createdByName: activeAccount.username, status: 'Open' }} />
      </section>}
